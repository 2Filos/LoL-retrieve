...
*Update: The mystery of why the edits were not detected has been solved! During the recent modularization, `style.css` was unlinked from `matchups.html` and split into `theme.css` and `components.css`. Because we were making changes to the dead `style.css` file, the browser wasn't detecting them. I have now migrated the margin-right fixes and the height variable over to the correct `components.css` and `theme.css` files.*
